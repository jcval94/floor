...
def _fit_scaler(rows: list[dict]) -> tuple[list[float], list[float]]:
    ...
    for j, mean in enumerate(means):
        variance = (
            sum((vector[j] - mean) ** 2 for vector in vectors)
            / max(1, len(vectors))
        )
...
def _softmax(logits: list[float]) -> list[float]:
    ...
    exps = [
        math.exp(max(-50.0, min(50.0, value - max_logit)))
        for value in logits
    ]
...
def _validate_params(params: dict) -> None:
    ...
    if not all(
        isinstance(row, list) and len(row) == len(FEATURE_NAMES)
        for row in weights
    ):
...
def predict_week_probabilities(
    ...
    *,
    ...
    means = [
        _to_float(means_map.get(name), 0.0)
        for name in FEATURE_NAMES
    ]
    scales = [
        max(abs(_to_float(scales_map.get(name), 1.0)), 1e-6)
        for name in FEATURE_NAMES
    ]
    ...
    logits = [
        _to_float(bias[k])
        + sum(
            _to_float(weights[k][j]) * vector[j]
            for j in range(len(FEATURE_NAMES))
        )
        for k in range(N_CLASSES)
    ]
...
def _cross_entropy(rows: list[dict], params: dict) -> float:
    ...
    losses = []
    for row in eligible:
        probs = predict_week_probabilities(
            row,
            params,
...
def _select_hyperparameters_with_cv(
    ...
    if not folds_data:
        reason = (
            ...
            if _has_integrity_metadata(train_rows)
...
def train_floor_week_m3_timing_model(
    ...
    if not train_eligible:
        raise ValueError(
            "No leakage-safe complete m3 timing rows available for training"
        )

    if training_mode == "retrain":
        params, tuning_summary = _select_hyperparameters_with_cv(
            train_eligible,
            folds=3,
        )
    else:
        ...
        reason = (
            "disabled_to_prevent_fold_leakage"
            if _has_integrity_metadata(train_eligible)
            else "not_requested"
        )
        tuning_summary = {
            "cv_enabled": False,
    ...
    outcomes = [
        1
        if (max(range(N_CLASSES), key=lambda i: probs[i]) + 1)
        == int(row["floor_week_m3"])
        else 0
        for probs, row in zip(raw_probs, valid_eligible)
    ]
    calibrator = (
        ProbabilityCalibrator(bins=10).fit(confidence, outcomes)
        if raw_probs
        else ProbabilityCalibrator(bins=10)
    )
    ...
    probabilities = [
        predict_week_probabilities(row, params)
        for row in valid_eligible
    ]
    ...
    best = [
        max(range(N_CLASSES), key=lambda i: probs[i]) + 1
