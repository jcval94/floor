...
# Writers publish immutable run/attempt-scoped sets. Readers only select complete
# sets, so an interrupted multi-file upload cannot replace the last valid state.
...
state_publish_set() {
  ...
  if ! [[ "$run_id" =~ ^[0-9]+$ ]] || ! [[ "$run_attempt" =~ ^[1-9][0-9]*$ ]]; then
    echo "::error::GITHUB_RUN_ID/GITHUB_RUN_ATTEMPT are required for versioned state publication" >&2
    return 2
  fi
