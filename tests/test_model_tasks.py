...
def test_normalize_model_tasks_with_none_returns_default_tasks() -> None:
    assert normalize_model_tasks(None) == ["d1", "w1", "q1", "value", "timing"]


def test_normalize_model_tasks_with_empty_values_returns_default_tasks() -> None:
    assert normalize_model_tasks("") == ["d1", "w1", "q1", "value", "timing"]
    assert normalize_model_tasks([]) == ["d1", "w1", "q1", "value", "timing"]
...
def test_normalize_model_tasks_with_m3_alias() -> None:
    assert normalize_model_tasks("m3") == ["value", "timing"]
